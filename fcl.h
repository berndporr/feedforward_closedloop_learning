...
class FeedforwardClosedloopLearning
{
  ...
  public:
    ...
    FeedforwardClosedloopLearning (
        const int num_input, const std::vector<int> &num_of_neurons_per_layer);
    ...
    /** Sets the activation function of the Neuron
         **/
    void
    setActivationFunction (FCLNeuron::ActivationFunction _activationFunction);
    ...
    {
        assert (i <= n_neurons_per_layer.size ());
        return layers[i];
    };
    ...
    {
        return layers[n_neurons_per_layer.size () - 1];
    };

    /** Gets the number of inputs
	 * \return The number of inputs
	 **/
    int getNumInputs () { return (int)ni; }
