...
FCLNeuron::FCLNeuron (int _nInputs)
{
    ...
    mask = new unsigned char[(unsigned)nInputs];
...
void FCLNeuron::calcOutput ()
{
    ...
    for (int i = 0; i < nInputs; i++)
    {
        ...
        if (*maskp)
        {
            ...
            {
                fprintf (stderr,
                         "Out of range Neuron::%s step=%ld, L=%d, N=%d, %f, "
                         "%f, %f, %d\n",
                         __func__, step, layerIndex, neuronIndex, sum,
                         weights[i], inputs[i], i);
            }
#endif
        }
        weightsp++;
    ...
    if (fabs (sum) > SUM_ERR_THRES)
        fprintf (stderr,
                 "Neuron::%s, Sum (%e) is very high in layer %d, neuron %d, "
                 "step %ld.\n",
    ...
    for (int i = 0; i < nInputs; i++)
    {
        ...
        if (*maskp)
        {
            *weightsp
                = *weightsp
                  + learningRate
                        * ((*inputsp) * error - fabs(error) * (*weightsp));
#ifdef DEBUG
            if (isnan (sum) || isnan (weights[i]) || isnan (inputs[i])
                || (fabs (sum) > SUM_ERR_THRES))
            {
